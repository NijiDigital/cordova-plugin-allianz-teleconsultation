...
#pragma mark CDVAllianzWKInAppBrowser

@interface CDVAllianzWKInAppBrowser () {
    NSInteger _previousStatusBarStyle;
}
@end

@implementation CDVAllianzWKInAppBrowser

static CDVAllianzWKInAppBrowser* instance = nil;
...
- (void)show:(CDVInvokedUrlCommand*)command withNoAnimate:(BOOL)noAnimate
{
    ...
    __block CDVAllianzInAppBrowserNavigationController* nav = [[CDVAllianzInAppBrowserNavigationController alloc]
                                                        initWithRootViewController:self.inAppBrowserViewController];
    nav.orientationDelegate = self.inAppBrowserViewController;
    nav.navigationBarHidden = YES;
    nav.modalPresentationStyle = self.inAppBrowserViewController.modalPresentationStyle;
    
    __weak CDVAllianzWKInAppBrowser* weakSelf = self;
...
{
    // Ensure a message handler bridge is created to communicate with the CDVAllianzWKInAppBrowserViewController
...
@end //CDVAllianzWKInAppBrowser

#pragma mark CDVAllianzWKInAppBrowserViewController

@implementation CDVAllianzWKInAppBrowserViewController
...
- (void)viewDidLoad
{
    allianzViewRenderedAtLeastOnce = FALSE;
    [super viewDidLoad];
}
...
- (void)viewDidDisappear:(BOOL)animated
{
    ...
    if (allianzIsExiting && (self.navigationDelegate != nil) && [self.navigationDelegate respondsToSelector:@selector(browserExit)]) {
        [self.navigationDelegate browserExit];
        allianzIsExiting = FALSE;
    }
}
...
{
    if (IsAtLeastiOSVersion(@"7.0") && !allianzViewRenderedAtLeastOnce) {
        allianzViewRenderedAtLeastOnce = TRUE;
...
@end //CDVAllianzWKInAppBrowserViewController
